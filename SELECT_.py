...
LEFT JOIN singers  ON singer_genre.singer_id = singers.singer_id    
GROUP BY genre.genre_name
ORDER BY count DESC;     
 

SELECT COUNT(records.record_name) 
FROM albums JOIN records ON albums.album_id = records.album_id
WHERE albums.release_year BETWEEN 2019 AND 2020;      
  

SELECT albums.album_name, AVG(records.record_duration) average 
FROM albums LEFT JOIN records ON albums.album_id = records.album_id
...
(SELECT singers.singer_name 
 FROM singers 
 FULL JOIN singer_album ON singers.singer_id = singer_album.singer_id
 FULL JOIN albums ON singer_album.album_id = albums.album_id
 WHERE albums.release_year = 2020 OR albums.release_year IS NULL);

 
SELECT collections.collection_name 
FROM collections
JOIN collection_record ON collections.collection_id = collection_record.collection_id
JOIN records ON collection_record.record_id = records.record_id     
JOIN albums ON records.album_id = albums.album_id    
...
JOIN singer_album ON albums.album_id = singer_album.album_id
JOIN singers ON singer_album.singer_id= singers.singer_id
JOIN singer_genre ON singers.singer_id= singer_genre.singer_id
GROUP BY albums.album_name, singers.singer_id 
HAVING COUNT(DISTINCT singer_genre.genre_id) >1;
  

SELECT records.record_name 
FROM records
FULL JOIN collection_record ON records.record_id = collection_record.record_id
